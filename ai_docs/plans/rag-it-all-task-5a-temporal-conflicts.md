...
- The manifest's `temporal_selector_json` stores the *pinned* selector, not `ResolvedTemporalSelector.selector_json`. The pure audit identity deliberately drops `known_at` for `resolved_known_at`, which is not a `TemporalSelector` under `extra="forbid"`, so the manifest and the `QuerySnapshot` that pins it both store the selector with its own `known_at` bound to the resolved cutoff. `selector_json` remains the conflict/audit identity; `pinned_selector(resolved)` produces the persisted form and `validate_knowledge_cutoff` then proves the two agree.
