...
- Conflict interval is the exact half-open intersection when provable; otherwise both bounds are null (amended 2026-09-11 after review). Status is `unresolved` when overlap is proven (including atemporal claims, which are timeless and therefore genuinely coexist) and `possible` when it cannot be proven. Assertion-version IDs and support-span IDs are sorted and unique before constructing the record. Amended 2026-09-11 after re-review: when any member's effective bounds carry a `temporal_precision` other than `instant`, neither overlap nor disjointness is proven, so the set is `possible` with null bounds exactly like an unknown-overlap case and no window-widening arithmetic is attempted.
- Independent sources remain represented in the set. A same-source ambiguous set can be `possible`; a same-source deterministically older version has already been removed from the current candidate set but remains queryable in history. Amended 2026-09-11 after re-review: `select_same_source` reports `superseded_version_ids` per series, so one version ID may be superseded in one series while remaining current in another; the integration slice (sections 4–6) must key supersession by series when persisting rather than treating the flat list as globally retired.
...
Amended 2026-09-11 in integration part 1, as `select_history` in `knowledge/temporal.py`:

- The service is `select_history(store, *, workspace_id, access, selector, request_cutoff, clock)` returning a `HistorySelection` (manifest, pinned selector, resolved selector, final `EvidenceSelection`, the per-row `HistoryDecision` inventory, the resolver and both proofs). `recorded_match` is reported on `HistoryDecision.recorded_reason`, which wraps an unchanged `match_temporal`; the pure predicates keep their reviewed behavior byte for byte.
- The manifest's `temporal_selector_json` stores the *pinned* selector, not `ResolvedTemporalSelector.selector_json`. The pure audit identity deliberately drops `known_at` for `resolved_known_at`, which is not a `TemporalSelector` under `extra="forbid"`, so the manifest and the `QuerySnapshot` that pins it both store the selector with its own `known_at` bound to the resolved cutoff. `selector_json` remains the conflict/audit identity; `pinned_selector(resolved)` produces the persisted form and `validate_knowledge_cutoff` then proves the two agree. Corrected 2026-09-11 after review: this held for four modes only, because `CurrentSelector` and `AtemporalSelector` had no `known_at` field to bind, so `pinned_selector` was the identity function for them and `validate_knowledge_cutoff` passed vacuously. Both now carry `known_at: datetime | None = None` (`None` still means "the latest knowledge available"), so all six modes round-trip: a pinned selector re-resolves to the pinned instant with origin `explicit` instead of to a replay's "now". Both fields are text columns, so schema 5 is still untouched.
- `HistoryManifest.identity_fields` gains `coverage_json`. Coverage is part of what a manifest asserts: two audiences can prove the same IDs from different proven/contextual inventories, and an identity that ignored the difference made one immutably overwrite the other. No column changes, so schema 5 is untouched.
- `coverage_json` is `{"proven": {...}, "contextual": {...}, "codes": [...]}` with per-kind counts and the sorted stable codes only. `retention_gaps` holds the selected revision IDs that no longer belong to any `active`/`retired` generation, so a collected generation is reported rather than fabricated.
- `TemporalInputs` is built from the record alone in this part. Binding `observed_at`, `source_updated_at` and `published_at` through support-group revisions is only needed by the `changes` clocks and is deferred; `None` stays honest for `as_of`, `during` and `current`.
- Purge: `store/snapshots.py` resolves `Suppression(reason="purge")` rows down to revisions, subtracts them before a history manifest can act as a collection root, and `purged_history_evidence(manifest_id)` returns `PurgedEvidence(target_kind, target_id, "evidence_purged")` markers that carry no retained text.
- A history `QuerySnapshot` pins `sources=()` and `profile_fingerprint="history:<manifest id>"`: it reproduces a manifest, not an embedding profile, so it can never be mistaken for current serving evidence. `settings_fingerprint` stays the caller's.
- N1 is closed by an optional `inherited_from` comparison parent on `ResolvedTemporalSelector`: `latest` must equal `latest_known_at`, `inherited` requires a parent whose cutoff matches and that contains this side, and no other origin may carry a parent.

Amended 2026-09-11 after the independent SPEC/QUALITY review of integration part 1 (`ai_docs/reports/2026-09-11-t5a-int1-review.md`), applied on `wp/t5a1fix`:

- Step 1's compare branch stays unimplemented in part 1. `select_history` rejects every `CompareSelector`, including one whose sides are independently pinned, and the error names the remedy: one independently pinned manifest per side, each with its own proof.
- Steps 2–5 run inside one `store.transaction()` against one authorization epoch. The broad proof, every row read, the closure scans and the `put_knowledge` are one atomic decision, so an interleaved publication or collection can no longer leave behind a manifest that no consistent state supported. Both proofs must carry the epoch captured at the start, exactly as `_reader_proof` requires.
- The invariant checked before the write is `manifest ⊆ proof` (revisions and assertion versions), not `proof ⊆ broad` alone: a narrowing between the two builds must stop the write, and the earlier check both ran in the wrong direction and ran after it. Both directions are now checked, before `put_knowledge`.
- Step 6's "revalidated" means the *requesting* audience. `acquire_history_snapshot` rebuilds the proof from its own `access` and `clock` through `history_access`, requires that proof to cover the manifest, and stores that resolver in the bundle, so no bundle revalidates a selection-time audience. Passing a manifest to an audience that cannot re-prove it raises `AuthorizationChanged` at acquisition rather than at first use.
- `history_unavailable` is a statement about retention only: it is emitted when the resolved cutoff precedes the earliest retained recorded interval among the audience's authorized rows. An authorized audience with no retained rows gets an empty manifest with no code, which leaks nothing and claims nothing.
- `HistoryManifest` and `ConflictSet` are `bookkeeping` in `RECORD_EPOCHS`, beside `QuerySnapshot`/`SnapshotReference`: they name evidence instead of being evidence. As `content` they fenced a read-only historical query behind the build lease of a source they only cite, and advanced `content_epoch` for a query that changed nothing.
- Adding `known_at` to the two implicit modes would otherwise have renamed every persisted record whose identity embeds a selector: `QuerySnapshot.identity_fields` contains the nested `temporal` model, so the canonical dump gained a `"known_at": null` and a stored row then failed to rehydrate with `Stored identity key disagrees with canonical record fields`. `Record.identity_parts` therefore omits a null `known_at` from the canonical parts at any depth: an unset cutoff is an absence, not a value, so every pre-change identity stays byte-identical, while a cutoff that *is* pinned still changes the identity, which is what pinning means. Verified against a `QuerySnapshot` JSON captured at `043ca51`.
- `purged_history_evidence(manifest_id, *, workspace_id, access)` is scoped to the caller. A foreign workspace, an unknown manifest and an audience that cannot prove the manifest's retained revisions all raise the same `SnapshotUnavailable`. The gate is the manifest *minus* its purged revisions, because a purge removes exactly the rows the markers describe from every proof.
...
- `knowledge/model.py`: canonical sorted/unique validation for `HistoryManifest` and `ConflictSet`; reject invalid conflict support/interval shapes while keeping unknown/open semantics. Do not add opaque provider ordering or authority fields to JSON. Amended 2026-09-11 in integration part 1: `canonical_ids` rejects unsorted or duplicated `HistoryManifest` revision/version/link/gap IDs and `ConflictSet` version/support IDs, and a `ConflictSet` upper bound without a lower bound is rejected because an unknown lower bound is not negative infinity; `valid_from` with an open `valid_to` stays valid.
- `knowledge/access.py`: expose a narrow history proof path that reuses existing source/policy/suppression and complete-support logic; never fork authorization policy. Preserve `query_mode` distinction exactly. Amended 2026-09-11 in integration part 1: the path is `EvidenceAccess.build_history(selection)`, which only pins `query_mode="history"` and delegates to `build`; no policy branch is duplicated.
- `knowledge/snapshots.py` and `store/snapshots.py`: acquire/validate history manifests and references atomically; recheck current authorization and include history reachability in collection roots. Amended 2026-09-11 in integration part 1: `acquire_history_snapshot(store, access, *, history, settings_fingerprint, lease_duration, clock)` returns the existing `QuerySnapshotBundle`, so validation, renewal and release are the one reviewed path; `acquire_query_snapshots` is unchanged.
