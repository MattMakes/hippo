...
- **PA2 review finding 4 → CC10, activation dispatch in `ai_docs/plans/rag-it-all-task-5-managed-code-capture.md`**, the slice that dispatches the code coordinator. `status.source_view` counts every `CODE_EDGE_KINDS` arrow whose source vertex is in the source's contributed node set, so a code edge on a shared symbol is attributed by node membership; only the relation counter below it checks `pair in row.source_generations`. It is unreachable while no managed code generation is served. Once code generations exist, and before managed code is served, `status.source_view` must attribute code edges by generation membership, gated on the exact selected pair as the relation counter is.
