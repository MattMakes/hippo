...
After successful preflight, run `_prepare_reindex` only for the legacy lane, completing all legacy clears before starting any job so the current orphan-sweep invariant remains true. Then submit each captured lane with its immutable actor and operation ID. One source's asynchronous failure must not clear another source, change another active pointer, or make another G1 unavailable. Keep the public API's `{"accepted": true}` response and do not expose hidden per-source outcomes. The internal integer remains number of jobs accepted by `Jobs`, not evidence counts. (Amended 2026-09-12 after the Task 3b review.) A managed preflight refusal is not "nothing to do": `reindex_all` raises a bounded `ManagedPreflightRefused` (a `ManagedDispatchError` naming no source and carrying no count), and the route maps it to `409 {"error": "Bulk reindex refused", "code": "bulk_refused"}`; an actorless bulk over a managed inventory raises `ManagedActorRequired` and maps to the generic permission response. Zero is returned only when there was genuinely nothing to reindex.
...
- Authenticated pasted text and `.md` upload publish strict managed G1. Open mode and an old library call remain legacy. Authenticated `.pdf`, `LICENSE` (no code or prose name), non-`.zip` archives and sample remain legacy; since CC10 of the managed code-capture plan, authenticated `.py` (any `readers.is_code_name` file), `.zip` and repo convert through the code coordinator. A managed source with no actor raises before any legacy delete hook.
...
- Mixed bulk contains a managed refresh, eligible legacy conversion, an unsupported legacy file (`LICENSE`; repo and archive became eligible with CC10), and tombstone. Failed managed preflight makes zero mutations. Successful preflight clears only legacy lanes, skips tombstone, preserves managed G1, and returns no hidden source breakdown.
...
Activation is one code/config behavior change with no background conversion sweep. Existing unmanaged sources remain legacy until an authorized eligible reindex. New authenticated text/plain ingestion opts in automatically, and since CC10 (`ai_docs/plans/rag-it-all-task-5-managed-code-capture.md`) so do authenticated code files, `.zip` archives and repositories; open mode stays legacy. This avoids rewriting existing evidence without an explicit source operation.
...
Five consequences of activation are release-note or contract material this task does not resolve; **Task 16 (Update/delete recovery, performance and release evaluation, `docs/rag_it_all.md`) owns all five**, so they are assigned here rather than in a scratch file:
...
- A gated `hippo index` originally set no `owner_id`/`access_role_id` for CLI-created sources; CLOSED at `39a3ba1` (`cmd_index` sets the owner and role). Task 16's release notes state the contract.
- `add_repo` originally took no `build_actor`, so a gated `hippo index <git-url>` stayed legacy; CLOSED by CC10 (`add_repo(build_actor, operation_id)`, `cli.py` passes the actor; `evidence-cc10.md`). What remains for Task 16 is that the LEGACY lane still stores a credentialed clone URL verbatim in `meta.url` (CC10 finding 2); the managed lane refuses such a URL before any row exists.
- `answer_withheld` still conflates a withheld answer with a failure that did save a trace (one that failed at the answer or grading step): that result reports a withheld answer and blanks the run summary. Deciding what a partially saved failed result owes a reader is an evaluation contract question (`evidence-pa4f.md`), not an activation defect.
- `summarize`'s gold means skip a failed question, so a run of nothing but routing failures reports `accuracy: None` beside `errors: N`. Pre-existing and correct; the evaluation `errors` card makes the pairing visible for the first time, so the release notes explain it.

The managed plain-prose ceiling stays at **1,000 chunks** (`PlainBuildOptions.max_chunks`) and does not track the legacy lane's `pipeline.MAX_CHUNKS` of 20,000 (`2026-09-11-pa3a-review.md` finding 8). The prose-coordinator plan states 1,000 as a bootstrap limit (§7), and a bootstrap installs in one transaction, so a larger source needs "a separately designed shadow-bootstrap state or reviewed bounds, not silent truncation". A managed build past the ceiling fails with a closed code and installs no managed state. Task 16's release notes say so, because a pasted document the legacy lane accepted can fail as a managed build.

Five review findings are correct today but need a file this task does not own. Each is assigned to the named owner, which makes the named change:

- **PA2 review finding 2 → Task 16, performance.** `knowledge/projection._selected_pairs` tests every authorized revision once per selected generation, O(selected generations × authorized revisions): latency, never a wrong result. When Task 16 benchmarks structural reads at its 10k/100k/1m sizes, it indexes the generation members once by generation (the review's `by_generation` shape).
- **PA2 review finding 4 → the `fix-pa2-4-code-edges` slice (`wp/pa2f4`, `evidence-pa2f4.md` beside this ledger)**, spawned when CC10 made the row reachable (CC10 merged at `9a474e4` without touching `status.py` and handed it back, `evidence-cc10.md` finding 1). `status.source_view` counts every `CODE_EDGE_KINDS` arrow whose source vertex is in the source's contributed node set, so a code edge on a shared symbol is attributed by node membership; only the relation counter below it checks `pair in row.source_generations`. It is unreachable while no managed code generation is served. Once code generations exist, and before managed code is served, `status.source_view` must attribute code edges by generation membership, gated on the exact selected pair as the relation counter is.
- **PA3a review finding 11 → Task 14**, the only later task whose Modify list names `ollama.py` (for profile support, which this is). `ingest/managed_activation.py` and `knowledge/dense_session.py` both import the private `ollama._base_name` and repeat `EMBED_PREFIXES.get(_base_name(...), ("", ""))`, and `ollama.py` holds a third copy. Task 14 adds one public helper beside `EMBED_PREFIXES` and moves both callers onto it.
- **PA3b review finding 6 → Task 9A.** `store/ladybug.py`'s restart sweep splices `REFRESHING_PREFIX` into the Cypher text (`STARTS WITH '…'`), where `store/memory.py` passes it as `$prefix`. Nothing is injectable today, but one predicate is spelled two ways. Task 9A's Modify list includes the stores and fakes, and it rebuilds the lease and recovery machinery this sweep belongs to; it passes the prefix as a query parameter on Ladybug. Task 5A, the only task whose Modify list names `store/ladybug.py`, is complete.
- **PA4b2 review finding 9 → Task 9**, the next task whose Modify list names `context.py`. `tests/unit/test_query_authorization_boundary.py` and `tests/unit/test_query_snapshots.py` monkeypatch the private `AppContext._build_structural_graph`. Task 9 gives `AppContext` a public build/observe seam and moves both tests onto it.
