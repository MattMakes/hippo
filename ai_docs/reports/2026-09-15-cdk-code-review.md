...
The branch moved while this review was being written: `37bad79` and `cbbe7b0` set the CK1-CK6
checkboxes, added the CK6 Neo4j parity line and updated the checkpoint. Neither touches `src/` or
`tests/` (`git diff --stat 0d50887..HEAD` is four Markdown files), so every line reference, count
and verdict below still reads at `0d50887`.
...
previous *generation* (not the previous revision's records) is what stays queryable. Read, not
inferred: `_bind` (`sync.py:1035-1049`) consumes only `_emit`'s `results` - the revisions that did
emit - and `_member_revisions` (`sync.py:803-822`) re-emits the parent's whole membership each run,
so no path carries the failed revision's records forward. `[M11]` runs one sync and asserts
publication plus the `emit_failed` count, so nothing pins the second-sync behaviour either way. Finding F14. Separately, `sync.py:1013-1017` attributes every raised
...
cannot be widened. `OWNED_BY` and `TRACKS` are the genuine hits in the "across (owner)" column R19
names; `SUPERSEDES` is a weaker within-column collision, recorded for completeness:
...
| `SUPERSEDES` | owner `{prose}`, decision and document kinds | listed in both the Prose **and** the Change sets "within" columns | A name collision in the *within* column rather than an owner disagreement in the "across (owner)" column R19 names, so it is the weaker of the three: a change connector wanting its own `SUPERSEDES` between two change-set nodes needs both a different owner family and endpoint kinds the built-in lacks, and gets `undeclared_owner_family` first |
...
CHECK line is Fake only, and the ledger's Scope makes LadybugDB the acceptance backend where
persistence is touched. `evidence-s6.md` records both files as LadybugDB-green, so the run exists
and the ledger does not name it. The Neo4j half of this gap closed while the review was being
written: the orchestrator recorded the CK6 parity run at `0d50887` in `neo4j-parity.md` (exit 0,
42 passed) in commit `37bad79`, after the baseline in section 1 was taken. What remains is the
LadybugDB CHECK line. Finding F8, narrowed. The ledger is the orchestrator's; nothing here changes
it.
...
| F5 | MINOR | `src/hippo/connectors/sync.py:202-211` | CK3; `evidence-s3c.md` open question 5 | `ensure_connector(..., enabled: bool = False)` is written to an existing row, so re-ensuring an instance to change its configuration silently disables it. R73 answered "pass `enabled=` deliberately", which makes every future caller carry the trap. Make it `enabled: bool | None = None`: `None` creates a row disabled (R51 unchanged) and leaves an existing row's value alone; `True`/`False` are explicit. The only two callers that pass it today are `cli.py:1171` (`hippo connector enable`) and `testing.py:1144` (`prepare_instance`), both `enabled=True`; R73's "S6's routes pass `enabled=` deliberately" names a route that does not exist yet (`POST /api/connectors` is Task 15's), so the change is cheap now and gets cheaper nowhere later. Add one test. Owner proposed: a S3-fix. Reruns CK3 and CK4 |
...
| F8 | MINOR | `ai_docs/gates/rag-it-all/cdk/GATES.md` CK6 | CK6 evidence | CK6 persists but has a Fake CHECK line only, although the ledger's Scope makes LadybugDB the acceptance backend where persistence is touched and `evidence-s6.md` records both files as LadybugDB-green. Narrowed while this review was written: `37bad79` added the CK6 Neo4j parity line at `0d50887` (exit 0, 42 passed). What remains is `CHECK: HIPPO_TEST_STORE=ladybug .venv/bin/pytest tests/unit/test_connector_exemplar.py tests/unit/test_connector_surfaces.py -q -o addopts='' -W error` on CK6, or a sentence in the Scope saying why CK6 is Fake-only. Owner: the orchestrator |
