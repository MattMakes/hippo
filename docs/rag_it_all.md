...
Generation input identity and extraction namespace propagation are implemented and independently reviewed in `ai_docs/gates/rag-it-all/task-5-namespace/GATES.md`: unchanged legacy IDs, separate logical source ownership, and consistent walker/resolver/Git/chunk references. Optional syntax and embedding cache primitives are reviewed in `task-5-syntax-cache/GATES.md` and `task-5-embedding-cache/GATES.md` under that same gates directory. Schema 4 storage now provides fenced builds, exact immutable membership, sealed publication, snapshot references, safe retries and conservative collection; `task-5-store/GATES.md` records the checks. Core query entry points hold one generation view through retrieval, model calls and rendering; `task-5-reader/GATES.md` records the checks. Schema 5 adds sealed rendered-view and inferred-prose lineage with complete input authorization. Projection and answer/search/analysis consumers now preserve original citations separately from retrieval candidates. Query sessions renew leases during model work; analysis and evaluation jobs hold their selected snapshots, and saved evaluation results retain exact snapshot references. Resolved model-profile and guarded embedding adapters are implemented but await production wiring. Immutable raw-object storage, bounded accepted byte/file capture, plain-reader mappings and provenance-aware prose chunk preparation are implemented and reviewed. Graph/code tools, source/status/account presentation and citation browsing now hold their selected snapshots through output. Plain chunk-to-evidence binding, evaluation/changeset lifetimes and the static fixture evaluator now have reviewed ownership and provenance checks. Controlled generation-profile binding, complete plain-prose model preparation and fenced staged writing are implemented and reviewed; `task-5-generation-profiles/GATES.md` and `task-5-prose-writer/GATES.md` record the checks. The captured OpenIE runtime verifies the chat model and authorization around every inference attempt; `task-5-openie-runtime/GATES.md` records its checks. Opt-in structural loading now preserves heterogeneous profile groups, legacy vectors, shared code objects, original-only typed objects and relation-only source support through audience scoping; `task-5-structural-loading/GATES.md` records the checks. Prospective build authority reuses the normal evidence policy engine before managed records exist, and a fenced failure transition releases an unpublished attempt without collecting it; `task-5-build-authority/GATES.md` and `task-5-build-failure/GATES.md` record those checks. Dense-session dispatch is published and reviewed; `task-5-dense-dispatch/GATES.md` records the checks. The plain-prose coordinator (detached atomic legacy bootstrap, fenced refresh, guarded receipts, non-collecting failure) is implemented and independently reviewed; `task-5-prose-coordinator/GATES.md` records the checks, including per-thread transaction ownership in every store. Production activation proceeds under `ai_docs/plans/rag-it-all-task-5-production-activation.md`: reviewed local workspace memberships, the managed tombstone barrier and the exact selected-generation source inventory are implemented and independently reviewed (`task-5-production-activation/evidence-pa1.md`, `evidence-pa2.md`); pipeline dispatch, route/actor activation and public error mapping remain. The full task remains open for code/rich/history capture and provenance. Existing ingestion is still the legacy pipeline.
...
The pure increment is implemented and independently reviewed: UTC half-open selectors with precision guards, adapter-declared ordering, per-adapter series, version-bucketed conflict sets and a validated chronological fixture; `ai_docs/gates/rag-it-all/task-5a-temporal-conflicts/GATES.md` records T5A1/T5A2/T5A5/T5A6 as met. History selection, append-only correction publication and storage integration (T5A3/T5A4) are not started; the plan file records the amended section 2–4 contracts.
