...
def test_v2_descriptor_and_checksum_are_frozen():
    ...
    assert m.CURRENT_SCHEMA_VERSION == 9
...
def test_new_store_keeps_all_migration_history_and_scope_freshness_is_independent(store):
    store.ensure_schema()
    assert [row["version"] for row in store.schema_history()] == [1, 2, 3, 4, 5, 6, 7, 8, 9]
    first = k.AccessPolicy(
...
def test_v3_transform_fault_keeps_legacy_policy_and_recovery_completes(store):
    ...
    if store.knowledge_backend == "fake":
        store._schema_history.pop(9)
        store._schema_history.pop(8)
        store._schema_history.pop(7)
        ...
        store.run("MATCH (v:SchemaVersion) WHERE v.version >= 3 DETACH DELETE v")
...
def test_pending_v3_recovery_never_rewrites_explicit_policy_identity(store, position):
    ...
    change_journal(store, 9, delete=True)
    change_journal(store, 8, delete=True)
    change_journal(store, 7, delete=True)
