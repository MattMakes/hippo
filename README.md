...
* **A collection or table named by a variable is invisible.** The same literals-only rule has a shape it is structurally blind to: a tool whose collection or table name comes from configuration — `db.Database(cfg.DB).Collection(coll.Name)`, an EF Core `DbSet` property like `_db.Orders` — names nothing literal in its own source, so it gets no data object and no `READS`/`WRITES` edge however well the method names (`InsertMany`, `Aggregate`, `SELECT`) are recognised. This is not a gap in the pattern list: a generic, config-driven data tool has no literal schema *because* it is generic. Measured on one: a 230-symbol Go repository produced zero `WRITES` edges and one `READS`, from the single literal collection name in the tree — in a test file. Ordinary applications, which name their tables in code, are unaffected.
...
* **History follows renames, but only by similarity.** A commit is attributed to a symbol by intersecting its diff with the symbol's line ranges *at that commit*, so line drift is not a source of error, and rename detection is on: a file that was renamed keeps the history it had under its old name, and a rename that changed nothing is credited with modifying nothing rather than with rewriting every symbol in the file. Renames are git's own similarity match, so a rename that rewrote more than half the file is a delete plus an add, and the history before it is not attributed. Commits that exceed `code_git_timeout_s`, or that fall outside the whole-pass `code_history_total_s`, are skipped and counted in `history_skipped`.
...
| `HIPPO_LADYBUG_CONNECTION_RECYCLE_STATEMENTS` | *(empty)* | How many statements with parameters one LadybugDB connection runs before hippo swaps in a fresh one on the same file, between transactions, because the driver keeps a copy of each until its connection closes. Empty means 4096, about 256 MiB at 64 KiB a statement; statements with large parameters count for more. Only a whole positive number is accepted. |
