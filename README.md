...
* **Some verbs are too common to guess from.** The bare-name fallback that earns a 0.50 `fuzzy_name` edge refuses a stoplist of about eighty ordinary method names — `get`, `close`, `write`, `log`, `main` — and the match is case-blind, so Go's `Close` and C#'s `Write` are refused exactly as Python's `close` and `write` are. Those calls are counted as unresolved rather than guessed at.
* **A name that means nine things seeds eight of them.** The split above is by the *true* number of matches, and only the first eight are kept, so a token past the cap under-spends its seed weight rather than redistributing it — nine matches means eight seeds of a ninth each. One repository written in five languages is enough to reach that: in hippo's own test fixture `log` matches nine symbols.
