...
from .model import SELF_NAMES, LanguageRules
...
#
# `self_names` / `super_names` are per language on purpose: `Self` is Rust's, and `base` --
# now on `csharp.RULES_ENTRY` -- is C#'s `super` but an ordinary Python module name: `from .
# import base` then `base.helper()` is a real INVOKES edge, and a shared `SUPER_NAMES`
# holding `base` would silently route it to the MRO instead.
