...
Three things about git that this module is built around, each verified on a real repository
rather than assumed:
...
def read_history(
    ...
    *,
    ...
    for position, entry in enumerate(entries):
        ...
        try:
            hunks = _hunks(checkout, entry["sha"], entry["parent"], timeout_s)
...
def _modifies(
    ...
    touched it. Rows are sorted by `(path, qualname)` so two runs agree.
    """
    ...
    return [rows[key] for key in sorted(rows, key=lambda k: rows[k]["hunk"]["file"])]
